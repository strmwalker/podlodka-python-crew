...
async def create_bill(session: AsyncSession, bill_in: Bill) -> models.Bill:
    ...
    users = await repositories.users.get_by_ids(
        session, [user_id for user_id in bill_in.amounts]
    )
    bill.shares = [
        models.BillShare(user=user, amount=bill_in.amounts[user.id]) for user in users
    ]
    session.add(bill)
    return bill
